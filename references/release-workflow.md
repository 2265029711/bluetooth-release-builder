...
11. 编译后定位 `out/<variant_key>/`。
12. 若 `source_bin_name` 已登记，直接使用该源 bin；若未登记，列出候选 `.bin` 并请求用户首次确认。
13. 按 `variant_key + change_tags + MMDD` 生成发布名建议。
14. 不主动重命名源 bin，只把建议名告诉用户。
15. 将确认后的修改位置、版本信息、源 bin 信息和说明更新到记录中。
16. 结束时给出修改汇总，至少包含：
   - 修改文件路径
   - 对应符号或函数
   - 改了什么值或什么逻辑
